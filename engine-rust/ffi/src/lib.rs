...
pub extern "C" fn aegis_canonicalize_url(input: *const c_char) -> *mut c_char {
    if input.is_null() {
        unsafe { aegis_set_last_error(1, b"null input to canonicalize\0".as_ptr() as *const c_char) };
...
pub extern "C" fn aegis_scan_buffer(buf: *const u8, len: size_t) -> i32 {
    if buf.is_null() || len == 0 {
        unsafe { aegis_set_last_error(1, b"null or empty buffer to scan\0".as_ptr() as *const c_char) };
        return 0;
    }
...
pub extern "C" fn aegis_verify_rulepack(pubkey: *const u8, pubkey_len: size_t, data: *const u8, data_len: size_t, sig: *const u8, sig_len: size_t) -> i32 {
    if pubkey.is_null() || data.is_null() || sig.is_null() {
        unsafe { aegis_set_last_error(1, b"null args to verify_rulepack\0".as_ptr() as *const c_char) };
        return 0;
    }
...
pub extern "C" fn aegis_scan_with_rulepack(buf: *const u8, len: size_t, rules_ptr: *const c_char) -> i32 {
    if buf.is_null() || len == 0 || rules_ptr.is_null() {
        unsafe { aegis_set_last_error(1, b"null arg to scan_with_rulepack\0".as_ptr() as *const c_char) };
        return 0;
    }
    let slice = unsafe { slice::from_raw_parts(buf, len as usize) };
...
#[no_mangle]
pub extern "C" fn aegis_verify_and_scan(pubkey: *const u8, pubkey_len: size_t, rulepack_ptr: *const c_char, rulepack_len: size_t, sig: *const u8, sig_len: size_t, buf: *const u8, buf_len: size_t) -> i32 {
    if pubkey.is_null() || rulepack_ptr.is_null() || sig.is_null() || buf.is_null() {
        unsafe { aegis_set_last_error(1, b"null arg to verify_and_scan\0".as_ptr() as *const c_char) };
        return 0;
    }
    let p = unsafe { slice::from_raw_parts(pubkey, pubkey_len as usize) };
    ...
    let rjson = match cstr.to_str() {
        Ok(s) => s,
        Err(_) => {
            unsafe { aegis_set_last_error(1, b"invalid rulepack json string\0".as_ptr() as *const c_char) };
            return 0;
        },
    };
    ...
    if !scanner::verify_rulepack_signature(p, rulepack_bytes, sigb) {
        unsafe { aegis_set_last_error(2, b"rulepack signature verification failed\0".as_ptr() as *const c_char) };
        return 0;
    }
    // proceed to scan
    let res = scanner::scan_with_rulepack(data_slice, rjson);
    if res == 0 {
        unsafe { aegis_log(b"scan returned no match\0".as_ptr() as *const c_char) };
    }
    res
}
...
pub extern "C" fn aegis_verify_and_scan(pubkey: *const u8, pubkey_len: size_t, rulepack_ptr: *const c_char, rulepack_len: size_t, sig: *const u8, sig_len: size_t, buf: *const u8, buf_len: size_t) -> i32 {
    ...
    // proceed to scan
    scanner::scan_with_rulepack(data_slice, rjson)
}
